...
intrinsics2 = np.array([[585.2304272868565, 0.0, 1190.0], [0.0, 585.2304272868565, 640.5], [0.0, 0.0, 1.0]])

extrinsics = np.array([[-0.11350664, -0.04971457, -0.99229265, 0.16266196],
...
def process_image(img, polygons, intr, extr):
    transform = intr @ extr
    print(transform)
    out = img.copy()
...
def main():
    ...
    img = process_image(image, polygons, intrinsics, extrinsics)
    cv2.imshow('img', img)
